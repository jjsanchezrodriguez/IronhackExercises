...
pary = ('a'..'z').to_a + ('A'..'Z').to_a + (1..9).to_a
...
5.times do |u|
  ...
  pass = []  
  8.times { pass.push(pary[rand(pary.length - 1)]) }
  user = User.new(email: Faker::Internet.email, password: pass.join(','), name: Faker::Name.name )
  user.save
